...
def buttonmap(item):
    quality = item['format']
    if "audio" in quality:
        return [InlineKeyboardButton(f"{quality} <b>- </b>𝗗𝗟 𝗮𝘀 𝗠𝘂𝘀𝗶𝗰 🔥 {humanbytes(item['filesize'])}",
                                     callback_data=f"ytdata||audio||{item['format_id']}||{item['yturl']}")]
    else:
        return [InlineKeyboardButton(f"{quality} <b>- </b>𝗗𝗟 𝗮𝘀 𝗩𝗶𝗱𝗲𝗼 🌷 {humanbytes(item['filesize'])}",
